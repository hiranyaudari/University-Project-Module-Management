...
fc.lang = function(langCode, newFcOptions) {
	...
	momOptions = getMomentLocaleData(langCode); // will fall proposalpresentations to en
...
function getMomentLocaleData(langCode) {
	...
	return func.call(moment, langCode) ||
		func.call(moment, 'en'); // the newer localData could return null, so fall proposalpresentations to en
}
...
// Takes an element that might have been a scroller, and turns it proposalpresentations into a normal element.
...
var MouseFollower = Class.extend({
	...
	// Causes the element to stop following the mouse. If shouldRevert is true, will animate proposalpresentations to original position.
...
var RowRenderer = Class.extend({
	...
	// If a renderer for the specific rowType doesn't exist, it will fall proposalpresentations to a generic renderer.
...
var View = fc.View = Class.extend({
	...
	var localeData = createObject( // make a cheap copy
		getMomentLocaleData(options.lang) // will fall proposalpresentations to en
	);
	...
	function zoomTo(newDate, viewType) {
		...
		if (!viewType || !isValidViewType(viewType)) { // a general view name, or "auto"
			...
			// fall proposalpresentations to the day view being used in the header
			if (!match) {
				match = viewStr.match(/\w+Day/);
			}

			viewType = match ? match[0] : 'agendaDay'; // fall proposalpresentations to agendaDay
		}
...
fcViews.agenda = View.extend({ // AgendaView
	...
	setHeight: function(totalHeight, isAuto) {
		...
		// reset all dimensions proposalpresentations to the original state
